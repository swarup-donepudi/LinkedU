...
public class SearchDAO extends AppDBInfoDAO {
    ...
    public void retrieveUniversitySearchResults(UniversitySearchCriteria usc, ArrayList<UniversityProfile> universitySearchResults) throws SQLException, UnsupportedEncodingException, IOException {
        String keyname = null;
        String univName=usc.getUnivName();
        ...
        try {
            ...
            String serviceQuery = "SELECT \"INSTNM\" from \"38625c3d-5388-4c16-a30f-d105432553a4\" "
                    + "WHERE LOWER(\"INSTNM\") LIKE '%" + univName + "%'";
        ...
        UniversityProfile univProfile=null;
        while (jsonParser.hasNext()) {
            switch (jsonParser.next()) {
                case START_OBJECT:
                    univProfile=new UniversityProfile();
                    break;
                case KEY_NAME:
                ...
                case VALUE_STRING:
                    if (keyname != null) {
                        if (keyname.equals("INSTNM")) {
                            ...
                            univProfile.setUniversityName(valueStr);
                        }
                    }
                    break;
                case END_OBJECT:
                    if(!univProfile.getUniversityName().equals(""))
                        universitySearchResults.add(univProfile);
                    break;
                default:
                    break;
            }
